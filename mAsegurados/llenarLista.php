    ...
    <script>
			...
			var listar = function(){
                ...
                "ajax":{
					...
					"columns":[
                        ...
                        {"defaultContent":"<button type='button' id='btnDesactivar' data-toggle='modal' data-target='#modalDesactivar' class='desactivar btn btn-sm btn-danger' ><i class='fas fa-trash-alt'></i></button>"}
                    ],
                stateSave: false,
                  dom: 'Bfrtip',
                  lengthMenu: [
            ...
            var abrirModalEditar=function(tbody,table) {
                    $(tbody).on("click","button.editar",function(){
                        ...
                        console.log(data);
                            var id =$("#idE").val(data.id_asegurado),
                            nssocial =$("#nssocialE").val(data.nssocial),
                            nombre =$("#nombreCompletoE").val(data.nombre_asegurado),
                            nombre =$("#nombreE").val(data.nombre),
                            paterno =$("#paternoE").val(data.ap_paterno),
                            materno =$("#maternoE").val(data.ap_materno),
                            ciudad =$("#ciudadE").val(data.ciudad_procedencia),
                            direccion =$("#direccionE").val(data.direccion),
                            sexo =$("#sexoE").val(data.sexo),
                            telefono =$("#telefonoE").val(data.telefono),
                            fecha =$("#fecha_nacE").val(data.fecha_nacimiento),
                ...
                var funcion_desactivar=function(tbody,table) 
                {
                    $(tbody).on("click","button.desactivar",function(){
                        ...
                        var data = table.row($(this).parents("tr")).data();
                        console.log(data);
                            var id =$("#idU").val(data.id_asegurado);
                            console.log(id);
                    });
                };
                ...
                var desactivar = function(){
                    $("#aceptar").on("click",function(){
                        var id_aseguradoU=$("#idU").val();
                        var clase = $("#labelDesactivar").text('Desactivar');
                        $.ajax({
                            method:"POST",
                            url:"status.php",
                            data:{
                                "id_aseguradoU":id_aseguradoU
                            }
                        }).done(function(info){
